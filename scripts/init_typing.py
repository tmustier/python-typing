...
def create_pyright_config(level: str, project_dir: Path) -> int:
    ...
    if config_path.exists():
        print("  ⚠️  pyrightconfig.json already exists, backing up to pyrightconfig.json.bak")
...
def create_typing_findings(project_dir: Path) -> None:
    ...
    if findings_path.exists():
        print("  ⚠️  typing-findings.md already exists, skipping")
        return
...
'''
    ...
    print("  ✅ Created typing-findings.md")
...
def create_precommit_hook(project_dir: Path) -> None:
    ...
    if not git_dir.exists():
        print("  ⚠️  Not a git repo, skipping pre-commit hook")
        return
    ...
    hook_content = _get_hook_content()

    # Check for existing hook
    if hook_path.exists():
        print("  ⚠️  pre-commit hook exists, appending pyright check")
    ...
    print("  ✅ Added pre-commit hook (warns on type errors)")
...
def install_long_task_harness(project_dir: Path) -> None:
    ...
    if (harness_dir / "long-task-progress.md").exists():
        print("  ✅ long-task-harness already initialized")
        return
    ...
    if init_script.exists():
        subprocess.run(
            [sys.executable, str(init_script), str(project_dir)],
            capture_output=True
        )
        print("  ✅ Initialized long-task-harness")
    else:
        print(f"  ⚠️  long-task-harness skill not found at {lth_skill}")
        print("      Install from: https://github.com/tmustier/long-task-harness")
...
def install_ralph_wiggum() -> None:
    ...
    if ralph_dir.exists():
        print("  ✅ ralph-wiggum plugin already available")
        return
    ...
    try:
        ...
        print("  ✅ Installed ralph-wiggum plugin")
        print("      Restart Claude Code to activate")
    except subprocess.CalledProcessError:
        print("  ⚠️  Failed to clone ralph-wiggum plugin")
        print("      Install manually: git clone https://github.com/tmustier/claude-plugins-official ~/.claude/plugins/marketplaces/claude-plugins-official")
...
def main() -> int:
    ...
    print("\n✅ Setup complete!")
    print("\nNext steps:")
    print("  1. Run: npx pyright")
    print("  2. Start fixing errors (see SKILL.md for strategy)")
    if baseline > 0:
