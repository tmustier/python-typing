...
# TypedDict for pyright JSON output
PyrightDiagnostic = dict[str, Any]
PyrightOutput = dict[str, Any]


def run_pyright() -> Optional[PyrightOutput]:
    ...
    try:
        ...
        output: PyrightOutput = json.loads(result.stdout)
        return output
...
def analyze_errors(data: PyrightOutput) -> None:
    """Analyze and display error distribution."""
    diagnostics: list[PyrightDiagnostic] = data.get("generalDiagnostics", [])
    ...
    for diag in diagnostics:
        if diag.get("severity") == "error":
            rule: str = diag.get("rule", "unknown")
            rules[rule] += 1

            file_path: str = diag.get("file", "unknown")
    ...
    for rule, count in rules.most_common(10):
        pct: float = count / error_count * 100
...
def _suggest_starting_point(
    ...
    if quick_wins > 0:
        ...
        print("     Start with: reportMissingParameterType, reportReturnType")
    ...
    if stub_count > 20:
        ...
        print("     Check: pip install types-requests types-pyyaml etc.")

    # Suggest smallest file with moderate errors
    if files:
        # Find files with 5-20 errors (manageable chunks)
        moderate_files: list[tuple[str, int]] = [
            (f, c) for f, c in files.items() if 5 <= c <= 20
        ]
        if moderate_files:
